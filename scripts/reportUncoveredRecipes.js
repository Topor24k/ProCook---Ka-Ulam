...
function findMatchedCategory(searchText) {
...
async function reportUncovered() {
  ...
  try {
    ...
    const uncovered = recipes.filter((recipe) => {
      const searchText = buildSearchText(recipe);
      return !findMatchedCategory(searchText);
    });

    console.log(`📌 Total recipes: ${recipes.length}`);
    console.log(`⚠️  Uncovered recipes: ${uncovered.length}`);

    if (uncovered.length > 0) {
      console.log('\nSample uncovered recipes:');
