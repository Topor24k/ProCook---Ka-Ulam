...
async function normalizeCategories() {
  ...
  try {
    ...
    const recipes = await collection.find({}, { projection: { _id: 1, name: 1, category: 1, description: 1, tags: 1, mealType: 1 } }).toArray();

    if (recipes.length === 0) {
      console.log('⚠️  No recipes found to update.');
      return;
    }

    const operations = recipes.map((recipe) => {
      const searchText = buildSearchText(recipe);
      const mappedCategory = matchCategory(searchText);
    ...
    console.log('📌 Categories after normalization:');
